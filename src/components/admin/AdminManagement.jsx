...
const AdminManagement = () => {
  ...
  // Admin Card - Improved mobile layout
      ...
      <div className="flex justify-between items-start mb-4 sm:mb-5">
        <div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900 truncate">{admin.name}</h3>
        ...
        <button 
          type="button"
          onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowDropdown(showDropdown === admin._id ? null : admin._id); }}
        ...
        {showDropdown === admin._id && (
          <div className="absolute right-2 sm:right-6 top-12 sm:top-14 bg-white rounded-lg shadow-xl border py-2 sm:py-3 z-20 w-40 sm:w-48 text-sm sm:text-lg">
            <button
              type="button"
              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setSelectedAdmin(admin); setIsEditOpen(true); setShowDropdown(null); }}
            ...
            {admin.isDeleted ? (
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleRestoreAdmin(admin._id); setShowDropdown(null); }}
              ...
              <button
                type="button"
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (window.confirm("Disable this admin?")) { handleDeleteAdmin(admin._id); setShowDropdown(null); }}}
      ...
      <div className="space-y-2 sm:space-y-3 text-sm sm:text-base lg:text-lg text-gray-700">
        ...
        {admin.campus?.name && (
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <span className="truncate">{admin.campus.name}</span>
          </div>
        )}
        {admin.cnic && (
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <CreditCard className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" /> 
            <span className="truncate">{admin.cnic}</span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-gray-100">
      ...
      <div className="bg-white border-b shadow-sm md:md:sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-indigo-700">Admin Management</h1>
              <p className="text-base sm:text-lg lg:text-xl text-gray-600 mt-1">Manage system administrators and their permissions</p>
            </div>
            <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl lg:rounded-2xl shadow-lg p-8 sm:p-12 lg:p-20 text-center">
            ...
            <h3 className="text-xl sm:text-2xl lg:text-3xl font-semibold text-gray-700 mb-2 sm:mb-3">No Admins Found</h3>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 sm:p-6" onClick={() => setIsCreateOpen(false)}>
          ...
          >
            <div className="flex justify-between items-center mb-6 sm:mb-8">
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Create New Admin</h2>
              <button 
      ...
      {isEditOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 sm:p-6" onClick={() => setIsEditOpen(false)}>
          ...
          >
            <div className="flex justify-between items-center mb-6 sm:mb-8">
              <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Edit Admin</h2>
              <button 
